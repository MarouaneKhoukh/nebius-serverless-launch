...
def lang_code_for_voice(voice: str) -> str:
    ...
    return {"af": "a", "am": "a", "bf": "b", "bm": "b", "jf": "j", "jm": "j", "zf": "z", "zm": "z", "ef": "e", "em": "e", "ff": "f", "hf": "h", "hm": "h", "if": "i", "im": "i", "pf": "p", "pm": "p"}.get(prefix, DEFAULT_LANG)
...
def synthesize(text: str, voice: str, speed: float) -> np.ndarray:
    chunks = [np.asarray(audio, dtype=np.float32) for _, _, audio in get_pipeline(lang_code_for_voice(voice))(text, voice=voice, speed=speed)]
    if not chunks:
...
def encode_audio(audio: np.ndarray, response_format: str) -> tuple[bytes, str]:
    ...
    proc = subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-i", "pipe:0", "-f", "mp3", "pipe:1"], input=wav_bytes, capture_output=True, check=False)
...
def list_models() -> dict[str, object]:
    return {"object": "list", "data": [{"id": API_MODEL, "object": "model", "owned_by": "nebius-serverless-cookbook"}]}
