"""OpenAI-shaped text-to-image server for Sana."""
...
def load_pipeline() -> SanaPipeline:
    ...
    pipe = SanaPipeline.from_pretrained(MODEL_ID, variant=MODEL_VARIANT or None, torch_dtype=torch.float16)
    pipe.to("cuda")
...
def list_models() -> dict[str, object]:
    return {"object": "list", "data": [{"id": MODEL_ID, "object": "model", "owned_by": "nebius-serverless-cookbook"}]}
...
def generate(req: GenerationRequest) -> dict[str, object]:
    ...
    try:
        with _gpu_lock:
            images = _pipe(prompt=req.prompt, negative_prompt=req.negative_prompt, width=width, height=height, num_images_per_prompt=req.n, num_inference_steps=steps, guidance_scale=guidance, generator=generator).images
